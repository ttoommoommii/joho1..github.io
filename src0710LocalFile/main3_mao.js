...
const modelUrl = "./mao/mao_pro_t01.model3.json";//main3
...
switch4.addEventListener("click", function (e) {	/* zoom　switch3が押された時の*/
	if (switch4.checked) {
		currentModel.scale.set((window.innerHeight - 50) / model_height * 2.5);
...
const rigFace = (result, lerpAmount = 0.7) => {
	...
	currentModel.internalModel.motionManager.update = (...args) => {
		...
		coreModel.setParameterValueById(
			"ParamMouthO",
			lerp(result.mouth.y, coreModel.getParameterValueById("ParamMouthO"), 0)   //0.3
		);
		/*coreModel.setParameterValueById(
			"ParamMouth",
			5.3 + lerp(result.mouth.x, coreModel.getParameterValueById("ParamMouth"), 0)
		);*/
	};
};
