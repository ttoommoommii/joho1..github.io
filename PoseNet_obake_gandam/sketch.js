...
// 宇宙人歩くNormally you would put this in a .json file, but I'm putting it here
...
function preload() {
        ...
        img = loadImage("img/bad.png");
        img2 = loadImage("img/obake.png");//("img/banana.png");
        img3 = loadImage("img/obake2.png");//("img/apple.png");
...
function setup() {
        ...
        explode_sprite = createSprite(camp_w - width / 4, 50, 171, 158);
        explode_sprite.addAnimation('explode', explode_animation);
        explode_sprite.scale = 0.6;
        // Create the Player sprite and add it's animations
        player_sprite = createSprite(100, camp_h - 96, 70, 94); //116
...
function draw() {
        ...
        if (mon == true) {
                ...
                image(img, noseX, noseY + 0.5 * w / 2, w * 3.0, w * 3.5);
                image(img2, rightHandX, rightHandY, w * 2, w * 2);
                image(img3, leftHandX, leftHandY, w * 2, w * 2);
                pop();
        ...
        if (explode_sprite.overlap(circle2)) {
                //mousePressed();
                //var s = createSprite(mouseX, mouseY, 30, 30);
        ...
        for (var x = 0; x < camp_w; x += 70) {
                tile_sprite_sheet.drawFrame('snow.png', x, camp_h - 50);//70
        }
        textSize(25);
        textAlign(CENTER, CENTER);
        let info = '手を動かし、白いおばけで、ほかのおばけをタッチ';
        fill(255,0,255);
        text(info, 310, camp_h - 20); 

        // Draw the sign tiles
        tile_sprite_sheet.drawFrame('signExit.png', camp_w - 80, camp_h - 120);//140
        tile_sprite_sheet.drawFrame('signRight.png', 0, camp_h - 120);//140
        ...
        if (camp_w - noseX > player_sprite.position.x + 10) { //x軸は反転なので
        ...
        else if (camp_w - noseX < player_sprite.position.x - 10) {
                player_sprite.changeAnimation('walk');
                // flip horizontally
                player_sprite.mirrorX(-1);
                // move right
